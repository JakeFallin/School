...
const initP2PServer = (p2pPort) => {
    const server = new WebSocket.Server({ port: p2pPort }); //Creates server 'WebSocket.server' and initializes p2p port
            ...
            */
            ...
            switch (message.type) {
                ...
                case MessageType.RESPONSE_BLOCKCHAIN://Message type Block Chain
                    const receivedBlocks = JSONToObject(message.data); //converts data of recieved blocks to JSON string of message.data
                ...
                case MessageType.RESPONSE_TRANSACTION_POOL:
                    const receivedTransactions = JSONToObject(message.data); //Converts transactions to JSON object
...
const handleBlockchainResponse = (receivedBlocks) => {
    ...
    const latestBlockHeld = blockchain_1.getLatestBlock(); //Determines what the latest block held is
    if (latestBlockReceived.index > latestBlockHeld.index) {
        ...
        if (latestBlockHeld.hash === latestBlockReceived.previousHash) {
            ...
            blockchain_1.replaceChain(receivedBlocks); //Replace the original chain with the received blocks 
        }
    }
    else {
...
const connectToPeers = (newPeer) => {
    const ws = new WebSocket(newPeer); //New websocket with newPeer to complete transaction with 
    ws.on('open', () => {
        initConnection(ws);
    });
    ws.on('error', () => {

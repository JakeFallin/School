...
(* 1. mapC: (int -> int) -> calcExp -> calcExp *)
let rec mapC func  = function
  | Const(c) -> Const(func c)
  | Add(c,d) -> Add(mapC func c, mapC func d)
  | Sub(c,d) -> Sub(mapC func c, mapC func d)
  | Mult(c,d) -> Mult(mapC func c, mapC func d)
  | Div(c,d) -> Div(mapC func c, mapC func d);;
...
(*This function returns the number of numbers in the list
that are even (or divisible by 2)  
 Eachother without a remainder, else it returns 1)
 (*f [5;30;120], [120;4], [2;10]  all return 2)
 ( f [5;12], [2;9], [133;34] all return 1 *)
 ( f [2;4;6;8] f [1;2;3;4;5;6;7;8] all return 4*)
